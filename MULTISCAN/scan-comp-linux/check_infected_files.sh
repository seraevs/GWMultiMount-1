...
echo $NOW "************************** BEGIN **************************" > $INFILE/logs/log_multiscan_linux.txt
...
echo $NOW " No infected files was found on scanned linux machines" 
else 
echo $NOW "Found infected files on scanned linux machines"  >> $INFILE/logs/log_multiscan_linux.txt
echo " Infected ===>$check_chr " >> $INFILE/logs/log_multiscan_linux.txt
cnt="1"
fi
...
if [ "$cnt" == "0" ]; then
echo $NOW " No infected files was found on all scanned linux machines" >> $INFILE/logs/log_multiscan_linux.txt
fi

echo $NOW "*************************** END ***************************"  >> $INFILE/logs/log_multiscan_linux.txt
